<?= $this->layout("layouts/app_layout", [
        "title" => $title ?? "Editar Cartão | " . APP_NAME,
        "active" => "cartoes",
]) ?>
...
$currentColor = old('color', $card->getColor() ?: '#696cff');
$currentIcon = old('icon', $card->getIcon() ?: '');
?>

<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="mb-6">Editar Cartão de Crédito</h4>

    <?= \App\Core\Message::render() ?>

    <div class="card">
        <div class="card-body">
            <form action="<?= url('/cartoes/' . $card->getId() . '/editar') ?>" method="post">
                <?= csrf_input() ?>
                ...
                <div class="row">
                    <div class="col-md-6 mb-6">
                        ...
                        <input type="text" class="form-control" id="name" name="name"
                               value="<?= old('name', $card->getName()) ?>" required/>
                    </div>
                    ...
                    <div class="col-md-6 mb-6">
                        ...
                        <input type="hidden" id="card_limit" name="card_limit"
                               value="<?= old('card_limit', $card->getCardLimit()) ?>"/>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-6">
                        ...
                        <input type="number" min="1" max="31" class="form-control" id="closing_day"
                               name="closing_day" value="<?= old('closing_day', $card->getClosingDay()) ?>" required/>
                    </div>

                    <div class="col-md-6 mb-6">
                        <label for="due_day" class="form-label">Dia de Vencimento</label>
                        <input type="number" min="1" max="31" class="form-control" id="due_day"
                               name="due_day" value="<?= old('due_day', $card->getDueDay()) ?>" required/>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="d-flex align-items-center gap-3 mb-3">
                        <span class="d-inline-flex align-items-center justify-content-center border rounded"
                            ...
                            <i id="icon-preview" class="icon-base bx <?= $currentIcon ?: 'bx-credit-card' ?> icon-md"></i>
                        </span>
                        <input type="text" class="form-control font-monospace" id="icon" name="icon"
                               data-default-icon="bx-credit-card"
                               value="<?= $currentIcon ?>" style="max-width: 220px;" placeholder="bx-credit-card"/>
                    </div>
                    <div class="d-flex flex-wrap gap-2">
                ...
                <div class="mb-6 form-check form-switch">
                    ...
                    <label class="form-check-label" for="is_active">Cartão ativo</label>
                </div>
